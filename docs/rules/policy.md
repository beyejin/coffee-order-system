...
외부 Mock 전송과 Redis read model 갱신은 결제 트랜잭션 커밋 후 실행한다. 외부 전송과 Redis 갱신 실패는 성공한 주문·잔액·이력을 롤백하지 않는다. 인기 메뉴 API는 Redis 오류 시 MySQL 집계로 fallback하며, MySQL이 주문·인기 메뉴의 정확성 기준이다. 현재 방식은 재시도와 영속 이벤트가 없어 프로세스 종료나 전송 실패 시 유실될 수 있으며, Outbox·Kafka는 이번 범위에 포함하지 않는다. 전략을 바꿀 때는 `strategy.md`와 `api-spec.md`를 먼저 갱신하고 구현한다.
