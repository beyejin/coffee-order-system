...
Docker Compose로 동일 애플리케이션 이미지의 `app1`, `app2`와 nginx gateway를 실제로 실행합니다. 두 앱은 세션·로컬 캐시를 사용하지 않고 하나의 MySQL을 공유하므로 어느 인스턴스로 요청이 가더라도 같은 잔액·주문·인기 집계 상태를 봅니다. MySQL·JDBC session·두 JVM은 모두 UTC를 사용합니다.

검증에서 nginx는 충전 요청을 `app2`, 주문 요청을 `app1`, 인기 메뉴 조회를 다시 `app2`로 전달했습니다. 공유 MySQL에는 최종 잔액 5,500P, 주문 1건, 포인트 이력 2건이 저장됐고 인기 메뉴도 주문 1건을 반환했습니다. 이는 로컬 메모리가 아니라 공유 DB가 정본이며, MySQL 비관적 락과 트랜잭션 경계가 인스턴스와 무관하게 적용됨을 보여줍니다.
...
**선택**: `@TransactionalEventListener(AFTER_COMMIT)` + `@Async`. 주문·차감·이력 저장이 커밋된 뒤 Mock 데이터 플랫폼으로 전송해 외부 지연이나 장애가 결제 응답과 DB 커밋을 되돌리지 않게 합니다.

**한계**: 메모리 이벤트라 애플리케이션이 커밋 직후 종료되거나 전송에 실패하면 재시도할 영속 기록이 없어 유실될 수 있습니다. 이번 과제에서는 결제와 외부 장애의 분리를 우선하며, 전송 보장 요구가 생기면 후속 단계에서 Outbox를 검토합니다. 현재 범위에는 Outbox·Kafka를 추가하지 않습니다.
...
**선택**: 실시간 `GROUP BY` 쿼리. 과제 규모에서는 조회 성능보다 정확성이 우선이며, 캐시나 별도 카운터의 정합성 문제를 추가할 근거가 없습니다.

주문 저장과 인기 메뉴 조회는 동일한 UTC `Clock`을 사용하고 시각을 MySQL `DATETIME(6)`과 같은 microsecond 정밀도로 절삭합니다. 조회마다 `to`를 한 번만 얻고 `[to - 168시간, to)` 구간을 집계합니다. 시작 경계는 포함하고 종료 경계는 제외해 연속 조회 구간이 겹치지 않게 합니다. JDBC 연결 세션과 로컬·테스트 MySQL도 UTC로 고정해 애플리케이션 시각과 DB 시각의 시간대 차이를 없앱니다.
...
주문 횟수 내림차순, 동률이면 메뉴 ID 오름차순으로 최대 3개를 반환하며 이름은 현재 `menu.name`을 사용합니다. `(created_at, menu_id)` 인덱스를 두고 실제 MySQL `SHOW INDEX`와 `EXPLAIN`을 확인하되, 작은 테스트 데이터에서 특정 실행계획 선택을 강제하지 않습니다.
