...
### 2.7 포인트 충전 금액 제약
0보다 큰 정수만 허용하며 상한선은 두지 않습니다. 요구사항에 명시되지 않은 기준(최대 충전 한도 등)을 임의로 만들지 않기 위함입니다.
...
## 5. 도전 요구사항: 동시성 / 데이터 일관성 / 실시간 전송 — 검토 중인 전략

아래 세 가지는 현재 학습 중인 주제로, 여러 후보안과 트레이드오프를 비교한 뒤 최종안을 확정할 예정입니다. (진행 상황은 이 문서를 계속 업데이트합니다.)
...
**현재 기본안**: MySQL 비관적 락. 단일 공유 DB 구조에서 추가 인프라 없이 다중 인스턴스에도 자연히 적용되는 가장 단순한 방법이기 때문입니다. 학습 후 Redisson 분산 락과 비교해 최종 결정합니다.
...
**현재 기본안**: 우선 구현 단계에서는 트랜잭션 내 동기 Mock 호출로 단순하게 시작하고, 학습 후 `AFTER_COMMIT` 비동기 방식으로 전환하는 것을 목표로 합니다.
...
**현재 기본안**: 실시간 `GROUP BY` 쿼리로 우선 구현하고, 이후 학습을 거쳐 필요 시 캐싱/카운터 방식으로 최적화 여부를 결정합니다. 과제 규모(현재 트래픽 수준)에서는 정확성이 성능보다 우선한다고 판단했습니다.
...
| 다중 인스턴스 증명 | docker-compose | 설계 주장에 그치지 않고 실제 동작을 시연 |
