...
Consumer는 `coffee-order-data-platform` Consumer Group으로 구독하고 `enable-auto-commit=false`, record ack mode를 사용합니다. Consumer concurrency는 Compose에서 3으로 설정해 Partition 수와 같은 수의 소비 스레드를 관찰할 수 있습니다. 실패 메시지는 `DefaultErrorHandler`의 짧은 재시도로 다시 처리하며, 재시도 한도를 넘긴 메시지를 DLT로 보내는 동작은 이번 범위에 포함하지 않습니다.
