...
기술 스택은 **Spring Boot / Java / MySQL / Redis**를 사용합니다. MySQL은 주문·포인트의 정본으로 유지하고, Redis는 인기 메뉴 조회를 위한 read model로 한정해 설계·구현의 깊이에 집중합니다.
...
| 다중 인스턴스 증명 | docker-compose | 동일 MySQL·Redis를 공유하는 2개 인스턴스의 동작을 시연 |
