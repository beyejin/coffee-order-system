...
| `domain/ranking` | 최근 7일 주문 집계와 상위 3개 조회 | `order`의 조회 저장소 |
...
- 이벤트는 랭킹 데이터를 별도로 갱신하지 않습니다. 인기 메뉴는 주문 원장을 조회 시점에 직접 집계합니다.
...
flowchart LR
    ...
    B -->|"UTC now - 7일 이상, now 미만"| C["OrderRepository.findPopularMenus"]
    C --> D["orders + menus 집계"]
    D --> E["주문 수 내림차순, menu id 오름차순, 상위 3개"]
```
...
| 주문 저장·시각 | 이벤트 발행 시점, 7일 집계 경계, 인덱스 | 주문·랭킹·migration 테스트 |
