...
8. PASS evidence와 PR URL이 모두 있을 때 기본 작업 완료를 보고합니다.
...
issue #4의 로컬 `main` 동기화 PR은 원격에 게시되지 않았던 기존 검증 커밋을 공개하는 일회성 복구입니다. repository owner가 전체 diff와 CI 결과를 수동 검토하며, merge 이후에는 재사용 가능한 예외로 취급하지 않습니다.
